...
ls_dir_only(Dir)->
	...
	L2=lists:map(fun(I) -> {I, file_type(Dir ++ "/" ++ I)} end, L),
	%%Get dirs only
	lists:filter(fun(J) -> case J of
						{_, {directory}} -> true;
						{_, _} -> false
							end end, L2).
...
pmap1_timeout(F,L,T)->
    ...
    Ref = erlang:make_ref(),
    foreach(fun(I) -> 
		    spawn_link(fun() -> do_f1(S, Ref, F, I) end)
	    end, L),
    %% gather the results
    gather1_timeout(length(L), Ref, [], T).
    
gather1_timeout(0, _, L, _T) -> L;
gather1_timeout(N, Ref, L,T) ->
    receive
	{Ref, Ret} -> gather1_timeout(N-1, Ref, [Ret|L],T)
    after T ->	L
    end.
